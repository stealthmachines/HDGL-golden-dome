...
/* ============================================================================
 * CH-2 CARRIER HANDLER — /serve/zc/<seg0>/<seg1>
 *
 * Intercepts requests whose path matches the phi-path pattern.
 * Decodes the carrier bit, accumulates into s_carrier.
 * Always responds 404 — identical to a legitimate cache miss.
 * The sender reads only the path; the response body is irrelevant.
 * ============================================================================ */
#include "zchg_carrier.h"
extern zchg_carrier_t s_carrier;
extern int            s_carrier_ready;

static int zchg_http_response_carrier_path(zchg_transport_server_t *server,
                                            zchg_http_connection_t  *conn)
{
    (void)server;
    if (s_carrier_ready) {
        int bit = zchg_carrier_path_decode(&s_carrier, conn->path);
        /* bit = 0 or 1: accumulated in s_carrier for drain via API.
         * bit = -1: not a valid carrier path (sync lost or genuine request).
         * Either way: 404. */
        (void)bit;
    }
    return zchg_http_build_response(conn, 404,
...
static int zchg_http_handle_request(zchg_transport_server_t *server, zchg_http_connection_t *conn) {
    ...
    /* ── CH-2 CARRIER: intercept before /serve/ ── */
    if (strncmp(conn->path, "/serve/zc/", 10) == 0) {
        return zchg_http_response_carrier_path(server, conn);
    }
